...
def getPlugins(inactive=False):
    ...
    # Get the contents of the pluin folder
    candidates = os.listdir(PluginFolder)

    # Loop over it
    for i in candidates:
        location = os.path.join(PluginFolder, i)

        # Ignore anything that does not meet our criteria
        if (not os.path.isdir(location) or
           PluginScript not in os.listdir(location)):
            continue

        # Load the module info into a variable
        ...
        if ScreenConf in os.listdir(location):
            ...
            if conf.get("enabled", False) or inactive:
                ...
                # See if there is a web config file
